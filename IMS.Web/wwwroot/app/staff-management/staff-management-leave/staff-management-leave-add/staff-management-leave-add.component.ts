...
export class AddStaffLeaveManagementComponent implements OnInit {
  ...
  addStaffLeave() {
    this.loaderService.toggleLoader(true);
    this.leave.FromDate = this.convertDateToUtc(this.leave.FromDate);
    this.leave.EndDate = this.convertDateToUtc(this.leave.EndDate);
    this.staffLeaveManagementService.addStaffLeave(this.leave).then(res => {
      var response = res.json();
      if (!response.hasError) {
        this.router.navigate(['staff', 'leavemanagement', 'list']);
      }
